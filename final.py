...
# Create a directory to store frames with gun detected if it doesn't exist
output_dir = r"D:\gun_final\detected_gun"
...
while True:
    ...
    if len(gun) > 0:
        ...
        # Save the frame with gun detected
        filename = os.path.join(output_dir, f"frame_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg")
    ...
    if gun_exist:
        ...
        # Display the frame with gun detected
        plt.imshow(frame)
